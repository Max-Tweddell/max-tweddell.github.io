...
function getVolume() {
  a = parseFloat(document.forms[1].a.value);
  b = parseFloat(document.forms[1].b.value);
  c = parseFloat(document.forms[1].c.value);
  height = parseFloat(document.forms[1].height.value);
  dbh = parseFloat(document.forms[1].dbh.value);
...
function calculateVolume(a, b, c, dbh, height) {
  return (
    Math.pow(dbh, a) *
    Math.pow(Math.pow(height, 2) / (height - 1.4), b) *
    Math.exp(c)
  );
}
function degToRad(V) {
...
function calculateDegHeight(top, datum, dist, datumht) {
  if (top > 50) {
    alert(
      "Angles greater than 50 degrees may produce poor estimates of height",
    );
  }
  ...
  height =
    Math.round(
      10 *
        (datumht +
          dist * (Math.tan(top) * Math.abs(Math.cos(datum) - Math.sin(datum)))),
    ) / 10;
  return height;
}
...
function calculatePercHeight(top, datum, dist, datumht) {
  if (
    numcheck(top, 50, 135, "Top angle") &&
    numcheck(datum, -50, 50, "Datum angle") &&
    numcheck(dist, 5, 120, "Distance") &&
    numcheck(datumht, 0, 5, "Datum Height")
  ) {
    if (top > 120) {
    ...
    height =
      Math.round(
        10 *
          (datumht + dist * ((top - datum) / Math.sqrt(10000 + datum * datum))),
      ) / 10;
  } else {
    height = "";
  }
